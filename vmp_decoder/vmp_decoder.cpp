...
#define xed_success(ret)                (ret == XED_ERROR_NONE)
    ...
    int vmp_decoder_run(struct vmp_decoder *decoder)
    {
        ...
        for (i = 0; i < num_of_inst; i++)
        {
            ...
            if (!vmp_start && vmp_addr_in_vmp_section(decoder, decoder->runtime_vaddr))
            {
                ...
                decoder->vmp_start_fa = decoder->runtime_vaddr - decoder->pe_mod->image_base;
    ...
    */
    ...
    static inline int vmp_x86_push_i32(struct vmp_decoder *decoder, int i32, unsigned char *data, int len)
    {
        return 0;
    }

    static int  vmp_x86_emulator(struct vmp_decoder *decoder, unsigned char *addr, int len)
    {
        uint32_t i32;
        uint16_t i16;
